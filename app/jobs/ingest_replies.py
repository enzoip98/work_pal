...
def run(the_date: date, persist: bool):
    ...
    print("—" * 60)
    print(f"Procesados: {processed} | Guardados: {saved} (persist={persist})")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Probar IA sobre respuestas de seguimiento")
    parser.add_argument("--date", help="Fecha YYYY-MM-DD (por defecto hoy)", default=None)
    parser.add_argument("--persist", action="store_true", help="Guardar tasks y marcar reply en BD")
    args = parser.parse_args()

    the_date = date.fromisoformat(args.date) if args.date else date.today()
    run(the_date, args.persist)